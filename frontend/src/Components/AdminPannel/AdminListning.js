        ...
        <div className="ad-card">
          <div className="ad-card__header">
            ...
            <div className="ad-search-wrap">
              ...
              <input className="ad-search-input" value={search} onChange={e => setSearch(e.target.value)} placeholder="Search title, address…" />
            </div>
          </div>

          {loading ? (
            <div className="ad-empty"><div className="ad-spinner" style={{ margin: "0 auto 12px" }} /><p>Loading…</p></div>
          ) : tab === "accommodation" ? (
            filteredAcc.length === 0 ? <div className="ad-empty">No accommodations found</div> : (
              <div style={{ overflowX: "auto" }}>
                <table className="ad-table">
                  <thead><tr><th>Title</th><th>Type</th><th>Price / mo</th><th>Gender</th><th>Status</th><th>Expires</th><th>Action</th></tr></thead>
                  <tbody>
                    {filteredAcc.map(a => (
                      <tr key={a._id}>
                        <td><div className="ad-cell-name">{a.title}</div><div className="ad-cell-sub">{a.address}</div></td>
                        ...
                        <td>
                          {a.isAvailable && !isExpired(a.expireDate)
            ...
            filteredFs.length === 0 ? <div className="ad-empty">No food services found</div> : (
              <div style={{ overflowX: "auto" }}>
                <table className="ad-table">
                  <thead><tr><th>Kitchen Name</th><th>Type</th><th>Delivery</th><th>Pickup</th><th>Status</th><th>Expires</th><th>Action</th></tr></thead>
                  <tbody>
                    {filteredFs.map(f => (
                      <tr key={f._id}>
                        <td><div className="ad-cell-name">{f.kitchenName}</div><div className="ad-cell-sub">{f.address}</div></td>
                        <td className="ad-cell-muted">{f.serviceType || "—"}</td>
                        <td>{f.deliveryAvailable ? <FaCheckCircle style={{ color: "#16a34a" }} /> : <FaTimesCircle style={{ color: "#dc2626" }} />}</td>
                        <td>{f.pickupAvailable  ? <FaCheckCircle style={{ color: "#16a34a" }} /> : <FaTimesCircle style={{ color: "#dc2626" }} />}</td>
                        <td>
                          {f.isAvailable && !isExpired(f.expireDate)
                            ? <span className="ad-badge ad-badge--active">Active</span>
                            : <span className="ad-badge ad-badge--expired">Inactive</span>}
                        </td>
