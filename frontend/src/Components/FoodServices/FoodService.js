  ...
  return (
    <div className="fs-gen-modal-overlay" onClick={onClose}>
      <div className="fs-gen-modal" onClick={(e) => e.stopPropagation()}>
        <div className="fs-gen-modal__icon fs-gen-modal__icon--warn">
          <FaExclamationCircle />
        </div>
        ...
        <div className="fs-gen-modal__actions">
          ...
          <button
            className="fs-gen-modal__btn fs-gen-modal__btn--confirm"
            onClick={onLogin}
          >
      ...
      <div className="fs-gen-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="fs-gen-modal__actions">
          ...
          >
            {deleting ? (
              <>
                <FaSpinner className="fs-spin" style={{ fontSize: 13 }} />{" "}
                Deleting…
              </>
            ) : (
              "Yes, Delete"
    ...
    >
      <div className="fs-review-modal">
        <div className="fs-review-modal__header">
          <div>
            <div className="fs-review-modal__title">
          ...
          <button className="fs-review-modal__close" onClick={onClose}>
            ✕
          </button>
        </div>
        <div className="fs-review-modal__body">
          ...
          <div className="fs-review-field">
            ...
            />
            <div className="fs-review-field__char-count">
              {text.length} / {MAX}
            </div>
          </div>
          <button
          ...
          >
            {submitting ? (
              <>
                <FaSpinner className="fs-spin" style={{ fontSize: 14 }} />{" "}
...
function ReviewCard({
  ...
  const joined = reviewer?.createdAt
  ...
  return (
    <div
      className={`fs-reviews__card${hasBorderBtm ? " fs-reviews__card--border-bottom" : ""}`}
    >
      <div
        className={
          isLeft
            ? "fs-reviews__card-inner-left"
            : "fs-reviews__card-inner-right"
        }
      >
        <div className="fs-reviews__author">
          ...
          {isOwn && (
            ...
            >
              <button
                className="fs-review-action-btn fs-review-action-btn--edit"
                onClick={onEdit}
                title="Edit review"
              >
              ...
              <button
                className="fs-review-action-btn fs-review-action-btn--delete"
                onClick={onDelete}
                title="Delete review"
              >
        ...
        <div
          className={`fs-reviews__text${!expanded && isLong ? " fs-reviews__text--clamped" : ""}`}
        >
          {review.comment}
        </div>
        {isLong && (
    ...
    >
      <div className="fs-menu-item__info">
        ...
        <div className="fs-menu-item__meta">
          <span className="fs-menu-item__meta-text">
            <FaClock style={{ fontSize: 11 }} /> {item.prepTime ?? 15} min prep
          </span>
          <span className="fs-menu-item__meta-text">
            <FaClock style={{ fontSize: 11 }} /> {openTime} – {closeTime}
          </span>
        </div>
        <div className="fs-menu-item__price-row">
          <span className="fs-menu-item__price">
      ...
      <div className="fs-menu-item__image" style={{ background: bg }}>
        {item.image ? (
          ...
          <span className="fs-menu-item__cat-icon">
            {CAT_ICON[item.category] ?? <FaUtensils />}
          </span>
        )}
        {availableNow && (
          <button
      ...
      {[
        {
          key: "delivery",
          label: "Delivery",
          icon: <FaMotorcycle style={{ fontSize: 13 }} />,
        },
        {
          key: "pickup",
          label: "Pickup",
          icon: <FaShoppingBag style={{ fontSize: 12 }} />,
        },
      ].map(({ key, label, icon }) => (
        <button
          key={key}
...
export default function FoodService() {
        ...
        setReviews((prev) =>
          prev.map((r) => {
            ...
            let photoUrl = photoId
              ? `${API_BASE}/Photo/${photoId}`
              : (r.reviewer?._profilePhotoUrl ?? null);
            return {
              ...r,
              reviewer: {
  ...
  useEffect(() => {
    ...
    fetch(
      `${API_BASE}/favourite/check/${userId}/${FOOD_SERVICE_ID}/FoodService`,
    )
      .then((r) => r.json())
      .then((raw) => {
        setIsFavourited(raw?.isFavourited === true);
      })
      .catch(() => {});
  ...
  const handleToggleFavourite = async () => {
    ...
    try {
      await fetch(`${API_BASE}/favourite`, {
        ...
        body: JSON.stringify({
          user: userId,
          itemId: FOOD_SERVICE_ID,
          itemType: "FoodService",
        }),
      });
      setIsFavourited((p) => !p);
      showToast(
  ...
  const scrollTo = (cat) => {
    setActiveNav(cat);
    sectionRefs.current[`cat-${cat}`]?.scrollIntoView({
      behavior: "smooth",
      block: "start",
    });
  };
  ...
  const mapLat = coords ? coords[1] : 6.902;
  ...
  const activeCategories = CATEGORIES.filter((cat) =>
    menuItems.some((i) => i.category === cat),
  );
    ...
    return (
      ...
      >
        ...
        <div style={{ fontSize: 18, fontWeight: 700 }}>
          No food service ID in URL
        </div>
        <div style={{ fontSize: 14, color: "#757575" }}>
          Expected: <code>/FoodService/&lt;id&gt;</code>
        </div>
      </div>
    );
  }
      ...
      <div style={{ padding: "0 24px" }}>
        <div className="fs-hero">
          {service?.BackgroundImage ? (
            <img
              src={photoSrc(service.BackgroundImage)}
              alt="banner"
              style={{
                position: "absolute",
                inset: 0,
          ...
          <nav className="fs-sidebar">
            {activeCategories.map((cat) => (
              <button
                ...
                style={{ fontFamily: FONT }}
              >
                <span className="fs-sidebar__btn-icon">{CAT_ICON[cat]}</span>{" "}
                {cat}
              </button>
            ))}
          </nav>
                ...
                return (
                  ...
                  >
                    <div className="fs-cat-section__title">
                      <span className="fs-cat-section__title-icon">
                        {CAT_ICON[cat]}
                      </span>{" "}
                      {cat}
                    </div>
            ...
            <div className="fs-cart__box">
              <div className="fs-cart__header">
                <div className="fs-cart__title">
                  Your order
                  {cartCount > 0 && (
                    <span className="fs-cart__count-badge">{cartCount}</span>
                  )}
                </div>
                ...
                {canDeliver && canPickup ? (
                  ...
                  <div
                    style={{ fontSize: 13, color: "#757575", marginBottom: 12 }}
                  >
                    {canDeliver ? (
                      <>
                        <FaMotorcycle style={{ marginRight: 4 }} /> Delivery
                        only
                      </>
                    ) : (
                      <>
                ...
                <div className="fs-cart__hint">
                  {orderType === "delivery" ? (
                    <>
                      <FaMotorcycle style={{ color: ORANGE }} /> Estimated
                      delivery: 30–45 min
                    </>
                  ) : (
                    <>
                      <FaShoppingBag style={{ color: ORANGE }} /> Ready for
                      pickup in 15–20 min
                    </>
                  )}
                </div>
              ...
              {cartItems.length === 0 ? (
                <div className="fs-cart__empty">
                  <div className="fs-cart__empty-icon">
                    <FaShoppingCart />
                  </div>
                  <div className="fs-cart__empty-title">
                    No items in your cart
                  </div>
                  <div className="fs-cart__empty-text">
                ...
                <>
                  {cartItems.map((item) => (
                    <div key={item._id} className="fs-cart__item">
                      ...
                      <div className="fs-cart__item-price">
      ...
      {modal && (
        ...
        >
          <div className="fs-item-modal">
            <div
              className="fs-item-modal__hero"
              style={{
                background:
                  BG_CYCLE[menuItems.indexOf(modal) % BG_CYCLE.length],
              }}
            >
              <button className="fs-item-modal__close" onClick={closeModal}>
                ✕
              </button>
              {modal.image ? (
                <img
                  src={photoSrc(modal.image)}
                  alt={modal.name}
                ...
                <span className="fs-item-modal__cat-icon">
                  {CAT_ICON[modal.category] ?? <FaUtensils />}
                </span>
              )}
            </div>
            <div className="fs-item-modal__body">
                ...
                {[
                  {
                    icon: <FaClock />,
                    label: "Prep Time",
                    val: `${modal.prepTime ?? 15} min`,
                  },
                  { icon: <FaTag />, label: "Category", val: modal.category },
                  {
                    icon: <FaClock />,
                    label: "Available Hours",
                    val: `${modal.AvailableHours?.open ?? "—"} – ${modal.AvailableHours?.close ?? "—"}`,
                  },
                  {
                    icon: <FaCheckCircle />,
                    label: "Status",
                    val: isItemAvailableNow(modal)
                      ? "Available now"
                      : "Not available",
                  ...
                  >
                    <span className="fs-item-modal__detail-label">
                      <span className="fs-item-modal__detail-icon">{icon}</span>{" "}
                      {label}
                    </span>
                    <span
                      className="fs-item-modal__detail-value"
                      style={color ? { color } : {}}
                    >
                      {val}
                    </span>
              ...
              <div className="fs-item-modal__price-row">
                <span className="fs-item-modal__price">
                  LKR {modal.price?.toLocaleString()}.00
                </span>
              </div>
              <div className="fs-item-modal__actions">
                <div className="fs-item-modal__qty-controls">
                  <button
                    className="fs-item-modal__qty-btn"
