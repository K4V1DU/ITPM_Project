...
// ── Pure helper: clamp all menu item times to fit within operating hours ─────
const clampMenuItemsToHours = (opOpen, opClose, items) =>
  items.map(item => {
    // From: clamp within [opOpen, opClose), must stay strictly before opClose
    let iOpen = clampTime(item.AvailableHours.open, opOpen, opClose);
    if (timeIdx(iOpen) >= timeIdx(opClose)) iOpen = opOpen;
    // Until: clamp within (iOpen, opClose], must stay strictly after iOpen
...
function EditFoodService() {
  ...
  const { id }         = useParams(); // food service ID from route
  ...
  const [deletedItemIds,   setDeletedItemIds]   = useState([]); // track removed existing items
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        // ── Helper: build photo URL directly from ObjectId ───────────────────────
        const photoUrl = (photoId) => `${BASE_URL}/photo/${photoId}`;

        // Step 3 – set photo previews directly from ID (no extra fetch needed)
        if (fs.iconImage) {
        ...
        // Step 4 – load existing menu items
        if (fs.menu && fs.menu.length > 0) {
          const itemsWithDetails = await Promise.all(
            fs.menu.map(async (itemId) => {
              ...
              const imgPreview = mi.image ? photoUrl(mi.image) : null;
              return {
                _id:           mi._id,
                name:          mi.name || "",
          ...
          // Clamp loaded menu item times to match loaded operating hours
          const opOpen  = fs.operatingHours?.open  || "08:00 AM";
          const opClose = fs.operatingHours?.close || "10:00 PM";
          setMenuItems(clampMenuItemsToHours(opOpen, opClose, itemsWithDetails));
          menuImageRefs.current = itemsWithDetails.map(() => null);
        }
  ...
  const removeMenuItem = (i) => {
    ...
    if (item._id && !item.isNew) {
      setDeletedItemIds(prev => [...prev, item._id]);
    }
  ...
  const handleMenuItemImageUpdate = async (i, e) => {
    ...
    try {
      const res = await axios.post(`${BASE_URL}/Photo`, fd);
      if (res.data.success) {
        // Only update state — do NOT delete old photo here
        setItemField(i, {
  ...
  const handleUpdateKitchenPhoto = async (e) => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        // Only update state — do NOT delete old photo here
        if (updatingField === "icon") { setIconPreview(preview); setIconImageId(newId); }
        else                          { setBgPreview(preview);   setBgImageId(newId); }
      }
  ...
  const handleDeleteKitchenPhoto = async (field) => {
    const photoId = field === "icon" ? iconImageId : bgImageId;
    try {
      await axios.delete(`${BASE_URL}/Photo/${photoId}`);
  ...
  const handleSaveListing = async () => {
    ...
    try {
      ...
      // 4. Update food service menu array
    ...
    return (
      <div className="container">
        <div className="error-state">
          ...
          <p>{loadError}</p>
          <button className="btn-next" onClick={() => navigate("/host")}>Go Back</button>
        </div>
      </div>
    );
  }
    ...
    <div className="container">
      ...
      {currentStep === 1 && (
        <div className="form-container">
          <h2 className="form-title">Kitchen Details</h2>
          ...
          <div className="form-group">
            ...
            <div className="service-option-row">
              <button type="button" className={`service-option-card ${deliveryAvailable ? "active" : ""}`}
              ...
              <button type="button" className={`service-option-card ${pickupAvailable ? "active" : ""}`}
      ...
      {currentStep === 2 && (
        <div className="form-container">
          <h2 className="form-title">Kitchen Location</h2>
          <p className="field-hint" style={{ marginBottom: 12 }}>Click on the map to update your kitchen location</p>
      ...
      {currentStep === 3 && (
        <div className="form-container">
          <h2 className="form-title">Kitchen Photos</h2>
          ...
          <div className="form-group">
            ...
            {!iconPreview ? (
              ...
              <div className="single-photo-preview clickable-preview" onClick={() => triggerKitchenUpdate("icon")}>
                <img src={iconPreview} alt="icon" className="preview-img icon-preview" />
                <div className="photo-click-overlay"><span>Click to change</span></div>
              </div>
            )}
          </div>
          ...
          <div className="form-group">
            ...
            {!bgPreview ? (
              ...
              <div className="single-photo-preview clickable-preview" onClick={() => triggerKitchenUpdate("bg")}>
            ...
            <div key={index} className="menu-item-card">
              <div className="menu-item-header">
                <div className="menu-item-header-left">
                  <h3>Item {index + 1}</h3>
                  {!item.isNew && <span className="existing-badge">Existing</span>}
              ...
              <div className="form-group">
                ...
                {!item.imagePreview ? (
                  ...
                  <div className="menu-item-img-preview clickable-preview" onClick={() => triggerMenuItemUpdate(index)}>
                ...
                <div className="form-group">
                  ...
                  <input type="number" className="form-input" value={item.price}
                    ...
                    onBlur={e => {
                      ...
                      if (v > 10000) updateMenuItem(index, "price", "10000");
                    }}
                ...
                <div className="form-group">
                  ...
                  <input type="number" className="form-input" value={item.prepTime}
                    ...
                    onBlur={e => {
                      ...
                      if (v > 120) updateMenuItem(index, "prepTime", "120");
                    }}
                    min="1" max="120" />
                </div>
              </div>
                  ...
                  <div className="time-picker-group">
                    ...
                    <select className="form-input time-select"
                      ...
                      onChange={e => {
                        ...
                        // if current close is no longer after new open, push it forward
                        if (closeIdx <= openIdx) {
                      ...
                      {TIME_OPTIONS
                        .filter(t => {
                          ...
                          // From: can be anything from service open up to (but not equal to) service close
                          // so there's always at least one valid "Until" option after it
                  ...
                  <div className="time-picker-group">
                    ...
                    <select className="form-input time-select"
                      ...
                      {TIME_OPTIONS
                        .filter(t => {
                          const i = TIME_OPTIONS.indexOf(t);
                          // Until: must be strictly after item open, up to and INCLUDING service close
        ...
        <div className="form-container">
          ...
          <div className="summary-card">
            ...
            {deletedItemIds.length > 0 && (
              <div className="summary-row deleted-notice">
                <span>Items to Remove</span>
                <strong>🗑️ {deletedItemIds.length} item(s) will be deleted</strong>
              </div>
            )}
          </div>
