    ...
    >
      <div className={`efs-modal ${wide ? "efs-modal--wide" : ""}`}>
        <div className="efs-modal-header">
          ...
          <button className="efs-modal-close" onClick={onClose}>
            <X size={16} />
          </button>
        </div>
...
export default function EditFoodService() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (fs.iconImage) {
          setIconPreview(photoUrl(fs.iconImage));
          setIconImageId(fs.iconImage);
        }
        if (fs.BackgroundImage) {
          setBgPreview(photoUrl(fs.BackgroundImage));
          setBgImageId(fs.BackgroundImage);
        }
        if (fs.menu?.length > 0) {
          const items = await Promise.all(
            fs.menu.map(async (itemId) => {
              ...
              return {
                ...
                AvailableHours: mi.AvailableHours || {
                  open: "08:00 AM",
                  close: "08:00 PM",
                },
  ...
  const useSLIITLocation = () => {
    ...
    if (mapInstance) {
      mapInstance.panTo(SLIIT_LOCATION);
      mapInstance.setZoom(17);
    }
  };
  const saveLocation = () => {
  ...
  const openAddMenuItem = () => {
    ...
    setMenuModal({ mode: "add", index: -1 });
  };
  const openEditMenuItem = (index) => {
    setEditingItem({ ...menuItems[index] });
    setMenuModal({ mode: "edit", index });
  };
  const removeMenuItem = (i) => {
    if (menuItems.length === 1)
      return alert("At least one menu item is required.");
    const item = menuItems[i];
    if (item._id && !item.isNew) setDeletedItemIds((p) => [...p, item._id]);
    setMenuItems((p) => p.filter((_, idx) => idx !== i));
  };
  const toggleMenuItemAvailable = (i) => {
    setMenuItems((prev) => {
      const u = [...prev];
      u[i] = { ...u[i], isAvailable: !u[i].isAvailable };
      return u;
    });
  };
  ...
  const saveMenuItem = () => {
    ...
    if (!editingItem.price || p < 30 || p > 10000)
      return alert("Price must be LKR 30–10,000.");
    if (Number(editingItem.prepTime) < 1 || Number(editingItem.prepTime) > 120)
      return alert("Prep time 1–120 min.");
    if (menuModal.mode === "add") {
      setMenuItems((prev) => [...prev, editingItem]);
    } else {
      setMenuItems((prev) => {
        const u = [...prev];
        u[menuModal.index] = editingItem;
        return u;
      });
    }
    setMenuModal(null);
  ...
  const handleSave = async () => {
    ...
    if (!iconPreview && !iconImageId)
      return alert("Please upload a kitchen icon.");
    ...
    try {
      ...
      await axios.put(`${BASE_URL}/Foodservice/${id}`, {
        ...
        deliveryAvailable,
        pickupAvailable,
        iconImage: finalIconId,
        BackgroundImage: finalBgId,
      });
      if (deletedItemIds.length > 0) {
        setSaveMsg("Removing deleted items…");
        await Promise.all(
          deletedItemIds.map((itemId) =>
            axios.delete(`${BASE_URL}/menuitem/${itemId}`),
          ),
        );
      }
      const menuItemIds = [];
  ...
  return (
    <div className="efs-root">
        ...
        <div className="efs-card">
          ...
          <div className="efs-section">
            {editingSection !== "details" ? (
              <div className="efs-section-header">
                <div>
                  <div className="efs-service-type-label">
                    {serviceType.toUpperCase()}
                  </div>
                  <h1 className="efs-kitchen-name">
                    {kitchenName || (
                      <span className="efs-placeholder">Kitchen name</span>
                    )}
                  </h1>
                  {description && (
                    <p className="efs-description">{description}</p>
                  )}
                </div>
              ...
              <div className="efs-inline-form efs-inline-form--has-icon">
                ...
                <div className="efs-field">
                  ...
                  <input
                    ...
                    placeholder="e.g. Mama's Home Kitchen"
                    maxLength={60}
                    autoFocus
                  />
          ...
          <div className="efs-section">
            {editingSection !== "hours" ? (
              <div className="efs-info-row">
                ...
                <button className="efs-edit-btn" onClick={startEditHours}>
                  <Pencil size={13} /> Edit
                </button>
              </div>
            ) : (
                ...
                <div className="efs-toggles-wrap">
                  <div className="efs-toggle-row">
                    ...
                    <Toggle
                      ...
                      onChange={(val) => {
                        if (!val && !pickupAvailable)
                          return alert(
                            "At least one of Delivery or Pickup must be active.",
                          );
          ...
          <div className="efs-section">
            <div className="efs-info-row">
              <div className="efs-info-left">
                ...
                <span className="efs-info-text">
                  {address || (
                    <span className="efs-placeholder">No location set</span>
                  )}
                </span>
              </div>
              <button className="efs-edit-btn" onClick={openMapModal}>
                <Pencil size={13} /> Edit
              </button>
            </div>
          </div>
          ...
          <div className="efs-section efs-section--menu">
            <div className="efs-menu-header">
              ...
              <button className="efs-add-menu-btn" onClick={openAddMenuItem}>
            ...
            {menuItems.length === 0 ? (
              <div className="efs-menu-empty">
                ...
                <button className="efs-btn-primary" onClick={openAddMenuItem}>
                ...
                {menuItems.map((item, i) => (
                  <div key={i} className="efs-menu-item">
                    ...
                    <div className="efs-menu-item-info">
                      <div className="efs-menu-item-name">
                        {item.name || "Untitled"}
                      </div>
                      <div className="efs-menu-item-meta">
                        <Clock size={12} className="efs-meta-clock" />
                        <span className="efs-meta-hours">
                          {item.AvailableHours?.open} –{" "}
                          {item.AvailableHours?.close}
                        </span>
                      </div>
                    ...
                    <div className="efs-menu-item-actions">
                      ...
                      <button
                        className="efs-menu-action-btn del"
            ...
            <div className="efs-card-actions-inner">
              {isSaving && saveMsg && (
                <span className="efs-save-progress">
                  <Loader2 size={13} className="efs-spin" />
                  {saveMsg}
                </span>
              )}
              <div className="efs-card-actions-btns">
                <button
                  className="efs-bottom-cancel-btn"
                  onClick={() => navigate("/Listings")}
                  disabled={isSaving}
                >
                  Cancel
      ...
      {showMapModal && (
        <Modal
          title="Set kitchen location"
          onClose={() => setShowMapModal(false)}
          wide
        >
          {mapLoadError ? (
            <div className="efs-map-error-inline">
              Map failed to load. Check your connection.
            </div>
          ) : !mapIsLoaded ? (
            <div className="efs-map-loading">
              <Loader2 size={20} className="efs-spin" color="#FF6B2B" />
            ...
            <>
              <div className="efs-map-wrapper">
                <GoogleMap
                  ...
                  center={draftLocation || SLIIT_LOCATION}
                  zoom={16}
              ...
              <div className="efs-map-actions">
                <button className="efs-map-btn" onClick={useSLIITLocation}>
                  <MapPin size={14} /> SLIIT University
                </button>
                <button className="efs-map-btn" onClick={useCurrentLocation}>
          ...
          <div className="efs-modal-footer">
            <button
              className="efs-btn-secondary"
              onClick={() => setShowMapModal(false)}
            >
              Cancel
            </button>
            <button className="efs-btn-primary" onClick={saveLocation}>
        ...
        >
            ...
            {!editingItem.imagePreview ? (
              ...
              <div className="efs-photo-preview">
                <img
                  ...
                  alt="item"
                  className="efs-item-img"
                />
                <div className="efs-photo-actions">
                  <button
                    type="button"
                    className="efs-icon-btn del"
                    onClick={() =>
                      setEditingItem((p) => ({
                        ...p,
                        imagePreview: null,
          ...
          <div className="efs-row">
              ...
              <input
                ...
                onBlur={(e) => {
                  ...
                  if (!isNaN(raw) && e.target.value !== "") {
                    const clamped = Math.min(10000, Math.max(30, raw));
                    setEditingItem((p) => ({ ...p, price: String(clamped) }));
                  }
                }}
                min="30"
                max="10000"
            ...
            <div className="efs-field">
              ...
              <input
                ...
                onChange={(e) =>
                  setEditingItem((p) => ({ ...p, prepTime: e.target.value }))
                }
                onBlur={(e) => {
                  const raw = Number(e.target.value);
                  if (!isNaN(raw) && e.target.value !== "") {
          ...
          <div className="efs-field">
            ...
            <div className="efs-hours-row">
              <div className="efs-field" style={{ flex: 1 }}>
                <label className="efs-label" style={{ fontSize: 10 }}>
                  From
                </label>
                <select
                  className="efs-select"
                  value={editingItem.AvailableHours.open}
                  onChange={(e) =>
                    setEditingItem((p) => ({
                      ...p,
                      AvailableHours: {
                ...
                >
                  {TIME_OPTIONS.filter(
                    (t) =>
                      timeIdx(t) >= timeIdx(operatingHours.open) &&
                      timeIdx(t) < timeIdx(operatingHours.close),
                  ).map((t) => (
                    <option key={t}>{t}</option>
                  ))}
                </select>
              ...
              <div className="efs-field" style={{ flex: 1 }}>
                ...
                <select
                  className="efs-select"
                  value={editingItem.AvailableHours.close}
                  onChange={(e) =>
                    setEditingItem((p) => ({
                      ...p,
                      AvailableHours: {
                ...
                >
                  {TIME_OPTIONS.filter(
                    (t) =>
                      timeIdx(t) > timeIdx(editingItem.AvailableHours.open) &&
                      timeIdx(t) <= timeIdx(operatingHours.close),
                  ).map((t) => (
                    <option key={t}>{t}</option>
                  ))}
              ...
              {DIETARY_TAGS.map((tag) => {
                ...
                return (
                  <button
                    key={tag.key}
                    type="button"
                    className="efs-tag-btn"
                    style={{
                      background: active ? tag.bg : "#f5f5f7",
                      color: active ? tag.color : "#aaa",
                      borderColor: active ? tag.border : "#e8e8e8",
          ...
          <div className="efs-field">
            <div className="efs-avail-row">
              ...
              <Toggle
                ...
                onChange={(v) =>
                  setEditingItem((p) => ({ ...p, isAvailable: v }))
                }
              />
            </div>
          </div>
          <div className="efs-modal-footer">
            ...
            <button className="efs-btn-primary" onClick={saveMenuItem}>
              <Check size={14} />{" "}
              {menuModal.mode === "add" ? "Add to menu" : "Save changes"}
            </button>
          </div>
        </Modal>
      )}
