...
import {
  ...
  AlertCircle, PenLine, Pencil, Save, Camera, Star,
  BookOpen, Settings, Clock3, MapPinned, Check, ChevronLeft,
} from "lucide-react";
...
const defaultMapOptions = { zoomControl: true, mapTypeControl: false, scaleControl: false, streetViewControl: false, rotateControl: false, fullscreenControl: true };
...
const emptyMenuItem = () => ({
  ...
  imagePreview: null, imageFile: null, imageId: null,
  isNew: true,
});
...
export default function EditFoodService() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (fs.menu?.length > 0) {
          const items = await Promise.all(fs.menu.map(async (itemId) => {
            const miRes = await axios.get(`${BASE_URL}/menuitem/${itemId}`);
            const mi    = miRes.data.data;
            return {
              _id: mi._id, name: mi.name || "", description: mi.description || "",
              price: mi.price?.toString() || "", category: mi.category || "Lunch",
              dietaryTags: mi.dietaryTags || [],
              AvailableHours: mi.AvailableHours || { open: "08:00 AM", close: "08:00 PM" },
              isAvailable: mi.isAvailable ?? true, prepTime: mi.prepTime || 15,
              imagePreview: mi.image ? photoUrl(mi.image) : null,
              imageFile: null, imageId: mi.image || null, isNew: false,
            };
          }));
          setMenuItems(items);
        }
  ...
  const startEditDetails = () => { setDraftName(kitchenName); setDraftDesc(description); setDraftType(serviceType); setEditingSection("details"); };
  const saveDetails = () => {
    ...
    setKitchenName(draftName); setDescription(draftDesc); setServiceType(draftType); setEditingSection(null);
  };
  const startEditHours = () => { setDraftOpen(operatingHours.open); setDraftClose(operatingHours.close); setEditingSection("hours"); };
  const saveHours = () => {
    if (timeIdx(draftClose) <= timeIdx(draftOpen)) return alert("Close must be after open.");
    setOperatingHours({ open: draftOpen, close: draftClose }); setEditingSection(null);
  };

  // ── Map ───────────────────────────────────────────────────────────────────────
  const openMapModal = () => { setDraftLocation(selectedLocation); setDraftAddress(address); setShowMapModal(true); };
  ...
  const useSLIITLocation = () => { setDraftLocation(SLIIT_LOCATION); if (mapInstance) { mapInstance.panTo(SLIIT_LOCATION); mapInstance.setZoom(17); } };
  const saveLocation = () => {
    if (!draftLocation) return alert("Please pin a location on the map.");
    if (!draftAddress.trim()) return alert("Please enter an address.");
    setSelectedLocation(draftLocation); setAddress(draftAddress); setHasLocation(true); setShowMapModal(false);
  };

  // ── Photos ────────────────────────────────────────────────────────────────────
  const handleIconSelect = (e) => { const f = e.target.files[0]; if (!f) return; setIconPreview(URL.createObjectURL(f)); setIconFile(f); e.target.value = null; };
  const handleBgSelect   = (e) => { const f = e.target.files[0]; if (!f) return; setBgPreview(URL.createObjectURL(f));   setBgFile(f);   e.target.value = null; };
  ...
  const toggleMenuItemAvailable = (i) => { setMenuItems(prev => { const u = [...prev]; u[i] = { ...u[i], isAvailable: !u[i].isAvailable }; return u; }); };
  const toggleEditingTag        = (tag) => { setEditingItem(prev => { const cur = prev.dietaryTags; return { ...prev, dietaryTags: cur.includes(tag) ? cur.filter(t => t !== tag) : [...cur, tag] }; }); };
  const handleEditingImageSelect = (e) => { const f = e.target.files[0]; if (!f) return; setEditingItem(prev => ({ ...prev, imagePreview: URL.createObjectURL(f), imageFile: f })); e.target.value = null; };
  const saveMenuItem = () => {
    ...
    if (menuModal.mode === "add") { setMenuItems(prev => [...prev, editingItem]); }
    else { setMenuItems(prev => { const u = [...prev]; u[menuModal.index] = editingItem; return u; }); }
  ...
  const handleSave = async () => {
    ...
    if (!iconPreview && !iconImageId) return alert("Please upload a kitchen icon.");
    if (!bgPreview && !bgImageId)     return alert("Please upload a cover image.");
    setIsSaving(true);
    try {
      let finalIconId = iconImageId;
      if (iconFile) { setSaveMsg("Uploading icon…"); const fd = new FormData(); fd.append("photo", iconFile); finalIconId = (await axios.post(`${BASE_URL}/Photo`, fd)).data.data._id; }
      let finalBgId = bgImageId;
      if (bgFile) { setSaveMsg("Uploading cover image…"); const fd = new FormData(); fd.append("photo", bgFile); finalBgId = (await axios.post(`${BASE_URL}/Photo`, fd)).data.data._id; }
      ...
      if (deletedItemIds.length > 0) { setSaveMsg("Removing deleted items…"); await Promise.all(deletedItemIds.map(itemId => axios.delete(`${BASE_URL}/menuitem/${itemId}`))); }
      const menuItemIds = [];
      for (let i = 0; i < menuItems.length; i++) {
        const it = menuItems[i];
        setSaveMsg(`Saving menu item ${i + 1} of ${menuItems.length}…`);
        let imageId = it.imageId;
        if (it.imageFile) { const fd = new FormData(); fd.append("photo", it.imageFile); const r = await axios.post(`${BASE_URL}/Photo`, fd); if (r.data.success) imageId = r.data.data._id; }
        const payload = { foodServiceId: id, name: it.name, description: it.description, price: Number(it.price), category: it.category, dietaryTags: it.dietaryTags, AvailableHours: it.AvailableHours, isAvailable: it.isAvailable, prepTime: Number(it.prepTime), ...(imageId && { image: imageId }) };
        if (it.isNew || !it._id) { const r2 = await axios.post(`${BASE_URL}/menuitem`, payload); menuItemIds.push(r2.data.data._id); }
        else { await axios.put(`${BASE_URL}/menuitem/${it._id}`, payload); menuItemIds.push(it._id); }
      }
    ...
    } finally { setIsSaving(false); setSaveMsg(""); }
  };
  ...
  if (isLoading) return (
    <div className="efs-root">
      <Topbar />
      <div className="efs-state-screen"><Loader2 size={32} className="efs-spin" color="#FF6B2B" /><p>Loading food service…</p></div>
    </div>
  );
  if (loadError) return (
    <div className="efs-root">
      <Topbar />
      <div className="efs-state-screen"><AlertCircle size={32} color="#dc2626" /><p className="efs-err">{loadError}</p><button className="efs-btn-primary" onClick={() => navigate("/Listings")}>Go Back</button></div>
    </div>
  );
    ...
    <div className="efs-root">
      ...
      <div className="efs-page">
        ...
        <div className="efs-hero">
          <div className="efs-cover-wrap">
            {bgPreview
              ...
              : <div className="efs-cover-empty"><ImageIcon size={28} /><span>Add cover photo</span></div>
            }
          ...
          <div className="efs-icon-wrap">
            <div className="efs-icon-circle">
              {iconPreview ? <img src={iconPreview} alt="icon" /> : <UtensilsCrossed size={30} color="#fff" />}
              <label className="efs-icon-edit-btn" htmlFor="efs-icon-upload"><Camera size={12} /></label>
        ...
        <div className="efs-card">
            ...
            {editingSection !== "details" ? (
              <div className="efs-section-header">
                <div>
                  ...
                  <h1 className="efs-kitchen-name">{kitchenName || <span className="efs-placeholder">Kitchen name</span>}</h1>
                  {description && <p className="efs-description">{description}</p>}
                </div>
                <button className="efs-edit-btn" onClick={startEditDetails}><Pencil size={13} /> Edit</button>
              </div>
            ) : (
              /* ── Extra padding-top to clear the overlapping icon circle ── */
              <div className="efs-inline-form efs-inline-form--has-icon">
                ...
                <div className="efs-field">
                  ...
                  <input className="efs-input" type="text" value={draftName} onChange={e => setDraftName(e.target.value)} placeholder="e.g. Mama's Home Kitchen" maxLength={60} autoFocus />
                ...
                <div className="efs-field">
                  ...
                  <textarea className="efs-textarea" value={draftDesc} onChange={e => setDraftDesc(e.target.value)} placeholder="Describe your kitchen…" maxLength={300} rows={3} />
                  <div className="efs-char-hint">{draftDesc.length}/300</div>
                </div>

                {/* Cancel + Save aligned to bottom-right */}
                <div className="efs-inline-form-footer">
                  <button className="efs-cancel-link" onClick={() => setEditingSection(null)}>Cancel</button>
                  <button className="efs-save-inline-btn" onClick={saveDetails}>
              ...
              <div className="efs-inline-form">
                ...
                <div className="efs-hours-row">
                  <div className="efs-field">
                    ...
                    <select className="efs-select" value={draftOpen} onChange={e => { const v = e.target.value; setDraftOpen(v); if (timeIdx(draftClose) <= timeIdx(v)) setDraftClose(TIME_OPTIONS[timeIdx(v) + 1] || v); }}>
                ...
                <div className="efs-inline-form-footer">
                  <button className="efs-cancel-link" onClick={() => setEditingSection(null)}>Cancel</button>
                  <button className="efs-save-inline-btn" onClick={saveHours}>
                    <Check size={14} /> Save changes
                  </button>
                </div>
              </div>
              ...
              <div className="efs-info-left">
                <div className="efs-toggles-wrap">
                  <div className="efs-toggle-row">
                    <div className="efs-toggle-label-wrap"><Truck size={16} className="efs-info-icon" /><span className="efs-toggle-label">Delivery</span></div>
                    <Toggle checked={deliveryAvailable} onChange={(val) => { if (!val && !pickupAvailable) return alert("At least one of Delivery or Pickup must be active."); setDeliveryAvailable(val); }} />
                  </div>
                  <div className="efs-toggle-row">
                    <div className="efs-toggle-label-wrap"><ShoppingBag size={16} className="efs-info-icon" /><span className="efs-toggle-label">Pickup</span></div>
                    <Toggle checked={pickupAvailable} onChange={(val) => { if (!val && !deliveryAvailable) return alert("At least one of Delivery or Pickup must be active."); setPickupAvailable(val); }} />
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="efs-section">
            <div className="efs-info-row">
              <div className="efs-info-left">
                ...
                <span className="efs-info-text">{address || <span className="efs-placeholder">No location set</span>}</span>
              </div>
                ...
                {menuItems.map((item, i) => (
                  <div key={i} className="efs-menu-item">
                    <div className="efs-menu-item-thumb">
                      {item.imagePreview ? <img src={item.imagePreview} alt={item.name} /> : <UtensilsCrossed size={20} color="#ccc" />}
                    </div>
                    <div className="efs-menu-item-info">
                      <div className="efs-menu-item-name">{item.name || "Untitled"}</div>
                      <div className="efs-menu-item-meta">
                        <span className="efs-meta-cat">{item.category}</span>
                        <span className="efs-meta-price">LKR {Number(item.price || 0).toLocaleString("en-US", { minimumFractionDigits: 2 })}</span>
                      </div>
                    </div>
                    <div className="efs-menu-item-actions">
                      <Toggle checked={item.isAvailable} onChange={() => toggleMenuItemAvailable(i)} />
          ...
          {/* ── In-card Save / Cancel (desktop only) ── */}
          <div className="efs-card-actions">
            <div className="efs-card-actions-inner">
              {isSaving && saveMsg && (
                <span className="efs-save-progress">
                  <Loader2 size={13} className="efs-spin" />{saveMsg}
                </span>
              )}
      ...
      {showMapModal && (
        <Modal title="Set kitchen location" onClose={() => setShowMapModal(false)} wide>
          {mapLoadError ? (
            ...
            <>
              <div className="efs-map-wrapper">
                <GoogleMap mapContainerStyle={mapContainerStyle} center={draftLocation || SLIIT_LOCATION} zoom={16} options={defaultMapOptions} onLoad={onMapLoad} onClick={onMapClick}>
          ...
          <div className="efs-field">
            ...
            <textarea className="efs-textarea" rows="2" value={draftAddress} onChange={e => setDraftAddress(e.target.value)} placeholder="Full address…" />
          </div>
      ...
      {menuModal && editingItem && (
        <Modal title={menuModal.mode === "add" ? "Add menu item" : "Edit menu item"} onClose={() => { setMenuModal(null); setEditingItem(null); }} wide>
          <div className="efs-field">
            ...
            {!editingItem.imagePreview ? (
              ...
              <div className="efs-photo-preview">
                ...
                <div className="efs-photo-actions">
                  <button type="button" className="efs-icon-btn del" onClick={() => setEditingItem(p => ({ ...p, imagePreview: null, imageFile: null, imageId: null }))}><Trash2 size={13} /></button>
                  <button type="button" className="efs-icon-btn upd" onClick={() => menuImgRef.current?.click()}><RefreshCw size={13} /></button>
                </div>
                <input type="file" accept="image/*" style={{ display: "none" }} ref={menuImgRef} onChange={handleEditingImageSelect} />
              </div>
            )}
          ...
          <div className="efs-row">
            <div className="efs-field">
              ...
              <input className="efs-input" type="text" value={editingItem.name} onChange={e => setEditingItem(p => ({ ...p, name: e.target.value }))} placeholder="e.g. Grilled Chicken Rice" />
            </div>
            <div className="efs-field">
              <label className="efs-label">Category</label>
              <select className="efs-select" value={editingItem.category} onChange={e => setEditingItem(p => ({ ...p, category: e.target.value }))}>
          ...
          <div className="efs-field">
            <label className="efs-label">Description</label>
            <input className="efs-input" type="text" value={editingItem.description} onChange={e => setEditingItem(p => ({ ...p, description: e.target.value }))} placeholder="Brief description…" />
          </div>
          <div className="efs-row">
            <div className="efs-field">
              <label className="efs-label">Price (LKR) * 30–10,000</label>
              <input className="efs-input" type="number" value={editingItem.price} onChange={e => setEditingItem(p => ({ ...p, price: e.target.value }))} onBlur={e => { const raw = Number(e.target.value); if (!isNaN(raw) && e.target.value !== "") { const clamped = Math.min(10000, Math.max(30, raw)); setEditingItem(p => ({ ...p, price: String(clamped) })); } }} min="30" max="10000" placeholder="350" />
            </div>
            <div className="efs-field">
              <label className="efs-label">Prep time * 1–120 mins</label>
              <input className="efs-input" type="number" value={editingItem.prepTime} onChange={e => setEditingItem(p => ({ ...p, prepTime: e.target.value }))} onBlur={e => { const raw = Number(e.target.value); if (!isNaN(raw) && e.target.value !== "") { const clamped = Math.min(120, Math.max(1, raw)); setEditingItem(p => ({ ...p, prepTime: String(clamped) })); } }} min="1" max="120" />
            </div>
          </div>
          <div className="efs-field">
            ...
            <div className="efs-hours-row">
              <div className="efs-field" style={{ flex: 1 }}>
                ...
                <select className="efs-select" value={editingItem.AvailableHours.open} onChange={e => setEditingItem(p => ({ ...p, AvailableHours: { ...p.AvailableHours, open: e.target.value } }))}>
                  {TIME_OPTIONS.filter(t => timeIdx(t) >= timeIdx(operatingHours.open) && timeIdx(t) < timeIdx(operatingHours.close)).map(t => <option key={t}>{t}</option>)}
                </select>
              </div>
              <span className="efs-hours-dash">–</span>
              <div className="efs-field" style={{ flex: 1 }}>
                <label className="efs-label" style={{ fontSize: 10 }}>Until</label>
                <select className="efs-select" value={editingItem.AvailableHours.close} onChange={e => setEditingItem(p => ({ ...p, AvailableHours: { ...p.AvailableHours, close: e.target.value } }))}>
                  {TIME_OPTIONS.filter(t => timeIdx(t) > timeIdx(editingItem.AvailableHours.open) && timeIdx(t) <= timeIdx(operatingHours.close)).map(t => <option key={t}>{t}</option>)}
                </select>
              </div>
            </div>
          </div>
              ...
              {DIETARY_TAGS.map(tag => {
                ...
                return (
                  <button key={tag.key} type="button" className="efs-tag-btn"
                    style={{ background: active ? tag.bg : "#f5f5f7", color: active ? tag.color : "#aaa", borderColor: active ? tag.border : "#e8e8e8", fontWeight: active ? 700 : 500 }}
          ...
          <div className="efs-modal-footer">
            ...
            <button className="efs-btn-primary" onClick={saveMenuItem}><Check size={14} /> {menuModal.mode === "add" ? "Add to menu" : "Save changes"}</button>
          </div>
        </Modal>
      )}
    </div>
