...
function ToastContainer({ toasts, onRemove }) {
  return (
    <div className="aac-toast-container">
      {toasts.map((t) => (
        <div
          key={t.id}
          className={`aac-toast aac-toast--${t.type} aac-toast--in`}
        >
          <span className="aac-toast__icon">{TOAST_ICONS[t.type]}</span>
          <span className="aac-toast__msg">{t.message}</span>
          <button className="aac-toast__close" onClick={() => onRemove(t.id)}>
            <X size={13} />
          </button>
        </div>
      ))}
...
const AddAccommodation = () => {
  ...
  const handleGetStarted = () => {
    if (!isLoggedIn) {
      addToast("Please log in before listing an accommodation.", "error");
      return;
    }
    setShowForm(true);
  };
  ...
  const validateStep1 = () => {
    let valid = true;
    if (rooms < 1 || rooms > 10) {
      setError("rooms", "Rooms must be between 1 and 10.");
      valid = false;
    }
    if (beds < 1 || beds > 10) {
      setError("beds", "Beds must be between 1 and 10.");
      valid = false;
    }
    if (bathrooms < 1 || bathrooms > 10) {
      setError("bathrooms", "Bathrooms must be between 1 and 10.");
      valid = false;
    }
    if (!valid)
  ...
  const validateStep2 = () => {
    ...
    if (!hasSelectedLocation) {
      setError("location", "Please pin your location on the map.");
      valid = false;
    }
    if (!address.trim()) {
  ...
  const validateStep3 = () => {
    ...
    if (!title.trim()) {
      ...
      setError("title", "Title cannot exceed 50 characters.");
      valid = false;
    }
    if (!description.trim()) {
      ...
      setError("description", "Description cannot exceed 200 characters.");
      valid = false;
    }
    if (!valid)
      addToast("Please fix the highlighted fields before continuing.", "error");
  ...
  const validateStep4 = () => {
    ...
    if (!isVerified) {
      setError("verify", "Please confirm accuracy.");
      valid = false;
    }
    if (!isAgreed) {
  ...
  const handleSaveListing = async () => {
    ...
    try {
      ...
      if (res.data) {
        addToast("Listing saved successfully! Redirecting…", "success", 3000);
  ...
  const Counter = ({ label, value, setter, min = 1, max = 10, errorKey }) => (
    <div
      className={`aac-counter${errors[errorKey] ? " aac-counter--error" : ""}`}
    >
      ...
      <div className="aac-counter__controls">
        <button
          ...
          onClick={() => {
            setter((v) => Math.max(min, Number(v) - 1));
            clearError(errorKey);
          }}
        ...
        <button
          ...
          className="aac-counter__btn"
          onClick={() => {
            setter((v) => Math.min(max, Number(v) + 1));
            clearError(errorKey);
          }}
            ...
            {currentStep === 2 && (
              <div className="aac-card">
                ...
                <div className="aac-field">
                  {mapsReady ? (
                    ...
                    <div className="aac-map-loading-placeholder">
                      <Loader2 size={15} className="aac-spin" /> Loading map
                      SDK…
                    </div>
                  )}
                </div>
                ...
                <div className="aac-map-wrapper">
                  {mapsReady ? (
                    ...
                    <div className="aac-map-skeleton">
                      <Loader2 size={22} className="aac-spin" />
                ...
                <div className="aac-field" style={{ marginTop: 16 }}>
                  ...
                  <textarea
                    className={`aac-textarea${errors.address ? " aac-input--error" : ""}`}
                    rows="2"
                    value={address}
                    onChange={(e) => {
                ...
                <div className="aac-row">
                  ...
                  <div className="aac-field">
                    ...
                    <input
                      ...
                      value={keyDuration}
                      onChange={(e) => {
                        setKeyDuration(e.target.value);
                        clearError("keyDuration");
                      }}
                      onBlur={(e) =>
