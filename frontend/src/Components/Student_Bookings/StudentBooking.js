...
import StudentNavbar from "../NavBar/Student_NavBar/StudentNavbar"; // adjust import path as needed
    ...
    <div className="sb-overlay" onClick={!loading ? onCancel : undefined}>
      <div className="sb-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="sb-modal__btns">
          ...
          <button
            className="sb-modal__btn sb-modal__btn--danger"
            onClick={onConfirm}
            disabled={loading}
          >
            {loading ? (
              <FaSpinner className="sb-spin" />
            ) : isDelete ? (
              "Delete"
            ) : (
              "Yes, Cancel"
...
// ─────────────────────────────────────────
// ACCOMMODATION ICON (for left list)
// ─────────────────────────────────────────
function AccomIcon({ booking, selected }) {
  ...
  if (src && !failed) {
    return (
      <div className="sb-row__icon sb-row__icon--img">
        <img
          src={src}
          alt={booking.accommodation?.title ?? "Property"}
          className="sb-row__icon-img"
  ...
  return (
    <div
      className={`sb-row__icon sb-row__icon--letter${selected ? " sb-row__icon--letter-active" : ""}`}
    >
      <FaHome />
    </div>
  );
...
function BookingRow({ booking, selected, onClick }) {
  ...
  return (
    <div
      className={`sb-row${selected ? " sb-row--active" : ""}`}
    ...
    >
      ...
      <div className="sb-row__body">
        ...
        <div className="sb-row__meta">
          {type && (
            <span>
              <FaBuilding style={{ fontSize: 9 }} /> {type}
            </span>
          )}
  ...
  return (
    <div className="sb-host-card">
      <div className="sb-host-card__avatar">
        {src && !imgFailed ? (
          <img
            src={src}
            alt={name}
            className="sb-host-card__avatar-img"
      ...
      <div className="sb-host-card__info">
        ...
        {phone ? (
          <a href={`tel:${phone}`} className="sb-host-card__phone">
...
// ─────────────────────────────────────────
// ACCOMMODATION SUMMARY CARD (detail panel)
// ─────────────────────────────────────────
      ...
      <div className="sb-accom-card__img-wrap">
        {src && !imgFailed ? (
          <img
            ...
            className="sb-accom-card__img"
            onError={() => setImgFailed(true)}
          />
        ) : (
          <div className="sb-accom-card__img-fallback">
            <FaHome />
          </div>
        )}
      </div>
        ...
        <div className="sb-accom-card__meta">
          {accommodation.accommodationType && (
            <span className="sb-accom-card__tag">
              <FaBuilding style={{ fontSize: 9 }} />{" "}
              {accommodation.accommodationType}
            </span>
          )}
          {accommodation.pricePerMonth && (
            <span className="sb-accom-card__tag sb-accom-card__tag--price">
              <FaTag style={{ fontSize: 9 }} /> LKR{" "}
              {Number(accommodation.pricePerMonth).toLocaleString()}/mo
            </span>
          )}
...
function BookingDetail({
  ...
  if (!booking) {
    return (
      <div className="sb-detail sb-detail--empty">
        ...
        <p className="sb-detail__empty-text">
  ...
  const handleMessageHost = async () => {
    const hostId =
      owner._id ??
      (typeof accommodation.owner === "string" ? accommodation.owner : null);
    if (!hostId || !currentUserId) {
      setMsgError("Cannot open chat — host info missing.");
      return;
    }
    setMsgLoading(true);
    setMsgError(null);
    try {
      ...
      console.error("[StudentBooking] message open failed:", err);
    } finally {
      setMsgLoading(false);
    }
  };
  ...
  return (
    <div className="sb-detail">
      ...
      <div className="sb-detail__booking-id">
        <span className="sb-detail__booking-id__text">
      ...
      <div className="sb-detail__header">
        ...
        <div className="sb-detail__header-row">
          ...
          >
            {msgLoading ? (
              <FaSpinner className="sb-spin" style={{ fontSize: 13 }} />
            ) : (
        ...
        <div className="sb-detail__header-meta">
          <span className="sb-header-meta__item">
      ...
      <div className="sb-detail__body">
        ...
        <div className="sb-detail__section">
          ...
          <div className="sb-dates-grid">
            <div className="sb-date-card">
              <div className="sb-date-card__label">
                <FaCalendarAlt className="sb-date-card__icon" /> Visit Date
              </div>
              <div className="sb-date-card__value">
                {formatDate(booking.visitDate)}
              </div>
            ...
            <div className="sb-date-card">
              <div className="sb-date-card__label">
                <FaClock className="sb-date-card__icon sb-date-card__icon--time" />{" "}
                Visit Time
              </div>
              <div className="sb-date-card__value">
                {booking.visitTime ?? "—"}
        ...
        <div className="sb-detail__actions">
          {canCancel && (
            <button
              className="sb-action sb-action--warning"
              onClick={() => onAction(booking, "cancelled")}
              disabled={busy}
            >
              {busy ? <FaSpinner className="sb-spin" /> : <FaBan />}
              Cancel Booking
            </button>
          )}
          <button
            className="sb-action sb-action--danger"
            onClick={() => onAction(booking, "delete")}
            disabled={busy}
          >
            {busy ? <FaSpinner className="sb-spin" /> : <FaTrashAlt />}
            Delete
          </button>
        </div>
      </div>
  ...
  const handleConfirmAction = async () => {
    ...
    try {
      if (action === "delete") {
        ...
        setSelectedBooking((prev) =>
          prev?._id === booking._id ? { ...prev, status: "cancelled" } : prev,
        );
    ...
    <div className="sb-page" style={{ fontFamily: FONT }}>
      ...
      <div className="sb-wrapper">
        ...
        <div className="sb-split">
          {/* LEFT */}
          <div
            className={`sb-split__left${isMobile && mobilePanel === "detail" ? " sb-split__left--hidden" : ""}`}
          >
            <div className="sb-searchbar">
              <div className="sb-search-wrap">
                ...
                <input
                  className="sb-search"
                  placeholder="Search property, address or message…"
                  value={searchQuery}
              ...
              <div className="sb-filter" ref={filterRef}>
                <button
                  className={`sb-filter__btn${statusFilter !== "all" ? " sb-filter__btn--active" : ""}`}
                  onClick={() => setFilterOpen((prev) => !prev)}
                >
                  <FaFilter style={{ fontSize: 11 }} />
                  <span className="sb-filter__label">{activeFilterLabel}</span>
                  <FaChevronDown
                    className={`sb-filter__chevron${filterOpen ? " sb-filter__chevron--open" : ""}`}
                  />
                </button>
                  ...
                  <div className="sb-filter__dropdown">
                    {FILTER_OPTIONS.map((opt) => {
                      const count =
                        opt.value === "all"
                          ? bookings.length
                          : bookings.filter((b) => b.status === opt.value)
                              .length;
                      ...
                      return (
                        ...
                        >
                          ...
                          <span
                            className={`sb-filter__count${statusFilter === opt.value ? " sb-filter__count--active" : ""}`}
                          >
                            {count}
                          </span>
                        </button>
              ...
              <div className="sb-active-filters">
                {statusFilter !== "all" && (
                  <span className="sb-filter-pill">
                    {activeFilterLabel}
                    <button
                      className="sb-filter-pill__remove"
              ...
              <div className="sb-empty">
                ...
                <div className="sb-empty__sub">
                  {isFiltered
                    ...
                    : "Your visit requests will appear here once you book a property"}
                </div>
                {isFiltered && (
                  <button
                    className="sb-empty__clear"
                    onClick={() => {
                      setSearchQuery("");
                      setStatusFilter("all");
                    }}
          ...
          {/* RIGHT */}
          <div
            className={`sb-split__right${isMobile && mobilePanel === "list" ? " sb-split__right--hidden" : ""}`}
          >
            <BookingDetail
      ...
      <div className={`sb-toast${toast.show ? " sb-toast--visible" : ""}`}>
        {toast.msg}
      </div>
    </div>
  );
